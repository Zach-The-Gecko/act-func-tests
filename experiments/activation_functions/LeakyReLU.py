...
def train_and_evaluate_model(epochs, train_loader, test_loader, model, loss_fn, optimizer, desired_accuracy=1):
    ...
    torch.save(model.state_dict(), "LeakyReLU/cifar10_LeakyReLU_model.pth")
    print("Model saved to LeakyReLU/cifar10_LeakyReLU_model.pth")
...
def get_batch_examples(num_examples, test_loader, model, loss_fn):
    ...
    with open('LeakyReLU/data_LeakyReLU.json', 'w') as f:
...
def main():
    # Create LeakyReLU output directory if it doesn't exist
    os.makedirs('LeakyReLU', exist_ok=True)
    ...
    model = LeakyReLUNeuralNetwork().to(device)
